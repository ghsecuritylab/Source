...
#if defined (CONFIG_GE1_RGMII_AN) || defined (CONFIG_GE1_MII_AN) || defined (CONFIG_GE2_RGMII_AN) || defined (CONFIG_GE2_MII_AN)
#undef MAC_TO_GIGAPHY_MODE_ADDR
unsigned char MAC_TO_GIGAPHY_MODE_ADDR = 0;
#endif

#ifdef CONFIG_ASUS_LEDDRV
int esw_port_event_link_up = 0;
EXPORT_SYMBOL(esw_port_event_link_up);
#endif
...
#if 1	/* ASUS++ for Repeater product: enable port6 VLAN tag field removal */
...
#else
	val &= 0xf003f003;
#if defined (CONFIG_MT7628_ASIC) /* temporarily use this CONFIG for RP-N12 */
	val |= 0x00140014;
#endif
...
#if defined (CONFIG_GE1_RGMII_AN) || defined (CONFIG_GE1_MII_AN) || defined (CONFIG_GE2_RGMII_AN) || defined (CONFIG_GE2_MII_AN)
static void vv_scan_PHY(void)
{
	unsigned int phy_id0, phy_id1;
	unsigned char try_addr, last_nonFF;
	last_nonFF = 0;

	for (try_addr=0; try_addr<32; try_addr++) {
		//printf("try:[%02x]  ", try_addr);
		if (!mii_mgr_read(try_addr, 2, &phy_id0)) {
			printk("\n Read PhyID 0 is Fail!!\n");
			phy_id0 = 0;
		}
		if (!mii_mgr_read(try_addr, 3, &phy_id1)) {
			printk("\n Read PhyID 1 is Fail!!\n");
			phy_id1 = 0;
		}
		//printf(" ph_id0:[%08x],phy_id1:[%08x]\n", phy_id0, phy_id1);
		if ((phy_id0 == 0x0143) && (phy_id1 == 0xBD63)) {
			printk(" ** Found Broadcom B50610, set PHY addr to %02x\n", try_addr);
			MAC_TO_GIGAPHY_MODE_ADDR = try_addr;
			break;
		}
		else if (phy_id0 == 0x1c) {
			printk(" ** Found Realtek PHY, set PHY addr to %02x, phy_id1:[%08x]\n", try_addr,phy_id1);
			MAC_TO_GIGAPHY_MODE_ADDR = try_addr;
			break;
		}
		else if ((phy_id0 != 0xFFFF) && (phy_id1 != 0xFFFF)) {
			printk(" ** Got something at addr[%02x]\n", try_addr);
			last_nonFF = try_addr;
		}
	}
	if (try_addr == 32)
		MAC_TO_GIGAPHY_MODE_ADDR = last_nonFF;
	printk("  Scan PHY Finish, set Addr to %02x\n",MAC_TO_GIGAPHY_MODE_ADDR);
}
#endif
