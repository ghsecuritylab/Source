...
*/
		...
		if(!nls)
		{
			...
			if ((ret=utf8s_to_utf16s(xfrstr, (int)strlen(xfrstr), unibuf)))
			{
				int i;
				len = 0;
...
_nvram_read(char *buf)
{
	...
	t1 = jiffies;
	valid_h->magic = 0;
	for (valid_offset = i = RESERVED_BLOCK_SIZE; i < nvram_mtd->size; i = ni) {
	    ni = i + STEP_UNIT;
	...
	t2 = jiffies;
	g_wlnv.next_offset = ROUNDUP(RESERVED_BLOCK_SIZE, STEP_UNIT);
	...
	if (cmt_times > 0 && valid_h->magic == NVRAM_MAGIC) {
		...
		g_wlnv.cur_offset = RESERVED_BLOCK_SIZE;
		if ((ret = MTD_READ(nvram_mtd, nvram_mtd->size - NVRAM_SPACE, NVRAM_SPACE, &len, buf)) ||
			...
			g_wlnv.next_offset = ROUNDUP(RESERVED_BLOCK_SIZE + valid_h->len, STEP_UNIT);
...
nvram_commit(void)
{
	...
	erasesize = ROUNDUP(NVRAM_SPACE + RESERVED_BLOCK_SIZE, nvram_mtd->erasesize);
	...
	/* Regenerate NVRAM */
	header = (struct nvram_header*) (commit_buf + RESERVED_BLOCK_SIZE);
	spin_lock_irqsave(&nvram_lock, flags);
	...
	if ((g_wlnv.next_offset + header->len) > nvram_mtd->size) {
		...
		g_wlnv.next_offset = RESERVED_BLOCK_SIZE;
	}

	unit_offset = g_wlnv.next_offset;
	erase_offset = unit_offset & ~(erasesize - 1);	/* align to lower erasesize boundary */
	...
	if (ret || len != rlen) {
		printk("nvram_commit: read error at 0x%x ret %d, len 0x%x/%x\n", unit_offset, ret, len, rlen);
		ret = -EIO;
		goto done;
	}
	...
	/* Erase sector blocks */
	if (need_erase) {
		if (erase_offset < RESERVED_BLOCK_SIZE) {
			restore_reserved_block = 1;
			len = 0;
			reserved_block = commit_buf;
			rlen = RESERVED_BLOCK_SIZE;
	...
	ret = MTD_WRITE(nvram_mtd, unit_offset, header->len, &len, (char*)header);
	if (ret || len != header->len) {
	...
	if (restore_reserved_block) {
		ret = MTD_WRITE(nvram_mtd, 0, RESERVED_BLOCK_SIZE, &len, reserved_block);
		if (ret || len != RESERVED_BLOCK_SIZE) {
			printk("nvram_commit: restore reserved block error. ret %d len 0x%x/%x\n", ret, len, RESERVED_BLOCK_SIZE);
			ret = -EIO;
			goto done;
		}
	}
	...
	g_wlnv.next_offset = ROUNDUP(unit_offset + header->len, STEP_UNIT);
	if (g_wlnv.avg_len)
