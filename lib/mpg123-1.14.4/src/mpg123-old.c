...
int asus_stop=0;
int roffset=0;
int process_offset(mpg123_handle *fr)
{
	roffset=0;
	do{
		if((roffset < 0) && (-roffset > framenum)) 
		   roffset = - framenum;
	} while (asus_stop);

	if(roffset)
	{
		if((roffset = mpg123_seek_frame(fr, roffset, SEEK_CUR)) >= 0)   
			debug1("seeked to %li", (long)roffset);
		else 
		   	error("seek failed");
	}
	return 0;
...
int process_sig(int sig)
{
   	asus_stop=1-asus_stop;
	ao->flush(ao);
	if(asus_stop)
		fprintf(stderr,"pause\n");
	else
		fprintf(stderr,"go on\n");
	return 0;
}   
...
int main(int sys_argc, char ** sys_argv)
{
	...
	mpg123_getpar(mp, MPG123_PREFRAMES, &param.preframes, NULL);
   	...
   	FILE *fp1;
        system("rm -rf /var/run/mpg123.pid");
        fp1= fopen("/var/run/mpg123.pid", "w");
        if(fp1 != NULL){
...
/* Rethink that SIGINT logic... */
		...
		while(!intflag)
		{
			...
			if (!have_dump) {
#ifndef NOXFERMEM
			   	dump_duration(mh,0,xfermem_get_usedspace(buffermem)); 
#else
				dump_duration(mh,0,0);
#endif
				have_dump=1;
				nvram_set("radio_preparing","0");
			}
			if(!param.quiet)
			{
	...
	if (play_ok)
		...
		safe_exit(1); /* That closes output and restores terminal, too. */
	return 0;
}
...
static void usage(int err)  /* print syntax & exit */
{
	...
	fprintf(o,"   					   --m3player   for RPN53 m3player\n");
