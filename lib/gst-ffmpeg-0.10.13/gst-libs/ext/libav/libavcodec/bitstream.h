...
extern const uint8_t libwma_log2_tab[256];


/*misc utility functions added to make it compile */
static inline int libwma_log2(unsigned int v)
{
    int n;
...
void libwma_align_put_bits(PutBitContext *s);
void libwma_put_string(PutBitContext * pbc, char *s, int put_zero);
...
int libwma_init_vlc(VLC *vlc, int nb_bits, int nb_codes,
...
#define DEBUGF(p, ...) printf(p, __VA_ARGS__)
