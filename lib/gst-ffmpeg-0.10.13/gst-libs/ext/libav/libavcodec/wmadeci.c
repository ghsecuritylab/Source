...
static int wma_decode_init_fixed(AVCodecContext * avctx)
{
    WMADecodeContextPrivate *s = avctx->priv_data;
    s->ectx = avctx;
    avctx->priv_data = s;
    int i, flags1, flags2;
    fixed32 *window;
...
//    s->block_align = avctx->block_align;
    ...
    if (s-> use_variable_block_len)
    {
        int nb_max, nb;
        nb = ((flags2 >> 3) & 3) + 1;
        if ((s->ectx->bit_rate / s->ectx->channels) >= 32000)
        {
            nb += 2;
        }
	...
	int fd,len;
	char ratebuf[30];
	fd=open("/tmp/irate",O_CREAT|O_TRUNC|O_WRONLY);
	if (fd!=-1) {
		len=snprintf(ratebuf,sizeof(ratebuf),"%d",sample_rate1);
		write(fd,ratebuf,len);
		close(fd);
	}
    }

    fixed64 tmp = itofix64(s->ectx->bit_rate);
    fixed64 tmp2 = itofix64(s->ectx->channels * s->ectx->sample_rate);
...
//    fprintf(stderr,"%s...\n",__func__);
   ...
   /* compute current block length */
    ...
    v = 0;
    for (ch = 0; ch < s->ectx->channels; ++ch)
    {
        a = get_bits(&s->gb, 1);
    ...
    for(ch = 0; ch < s->ectx->channels; ++ch)
    {
        ...
        if (s->version == 1 && s->ectx->channels >= 2)
        {
            align_get_bits(&s->gb);
        }
    }
    ...
    if (s->use_bit_reservoir) {
        ...
        if (s->last_superframe_len > 0) {
            ...
            if (wma_decode_frame(s, samples) < 0)
                goto fail;
            samples += s->ectx->channels * s->frame_len;
        }
...
AVCodec ff_wmav1_decoder =
	...
	AVMEDIA_TYPE_AUDIO,
        CODEC_ID_WMAV1,
        sizeof(WMADecodeContextPrivate),
