...
"$Id: plugin.c,v 1.3 2009-08-21 05:14:19 steven Exp $";
...
PPPoEDevnameHook(char *cmd, char **argv, int doit)
{
	...
	/* Strip off "nic-" */
	cmd += 4;
    } else if (strlen(cmd) < 3 || (strncmp(cmd, "eth", 3) && strncmp(cmd, "ra", 2) &&
		strncmp(cmd, "apcli", 5) && strncmp(cmd, "nas", 3) && strncmp(cmd, "tap", 3))) {
