...
"<#WLANConfig11b_TxPower_itemdesc#>(<#JS_validrange#> 1 <#JS_validrange_to#> 100)",
"WEP-64bits: <#WLANConfig11b_WEPKey_itemtype1#><br/>WEP-128bits: <#WLANConfig11b_WEPKey_itemtype2#>",
"",
"",
"",
"<div><#QIS_wireless_help1#></div><br/><img id='hc0_img' src='/images/qis/windows_wifi.png'>",
...
"<#WLANConfig11b_x_RadioEnableDate_itemdesc#><p><a href='/Main_LogStatus_Content.asp' target='_blank'><#General_x_SystemTime_itemname#><#CTL_go#></a></p>",
"<#WLANConfig11b_x_RadioEnableTime_itemdesc#><p><a href='/Main_LogStatus_Content.asp' target='_blank'><#General_x_SystemTime_itemname#><#CTL_go#></a></p>",
...
"<#WLANConfig11b_Roaming_itemdesc#>");
...
"<#LANHostConfig_x_GWStaticIP_itemdesc#>",
"<#LANHostConfig_x_GWStaticMask_itemdesc#>",
"<#LANHostConfig_x_GWStaticGW_itemdesc#>",
"<#LANHostConfig_x_GWStaticMT_itemdesc#>",
"<#LANHostConfig_x_GWStaticIF_itemdesc#>");
//WAN
...
function suspendconn(wanenable){
	document.internetForm_title.wan0_enable.value = wanenable;
	showLoading();
		...
		if(usb_path1 == "usb=" && usb_path2 == "usb="){
			...
			for(var i = 0; i < apps_array.length; i++){
				if(apps_array[i][0] == "downloadmaster" && apps_array[i][4] == "yes" && apps_array[i][3] == "yes"){
					//dmStatus = "Enabled";
					if(pool_names() != ""){ //  avoid no disk error
				...
				else if(apps_array[i][0] == "downloadmaster" && apps_array[i][3] == "no")
					dmStatus = "Not installed";		
				else if(apps_array[i][0] == "downloadmaster" && apps_array[i][4] == "no" && apps_array[i][3] == "yes")
					dmStatus = "Disabled";
			}
...
var usb_path_tmp = new Array('usb=<% nvram_get("usb_path2"); %>', 'usb=<% nvram_get("usb_path1"); %>');
        ...
        var usb_path_curr = eval("usb_path"+_foreign_disk_interface_name);
			...
			if(current_url!="index.asp" && current_url!=""){
				if((usb_path_tmp[0] != usb_path1) && usb_path1 != "usb=" && usb_path1 != "usb=printer")
					statusmenu += "<div style='margin-top:2px;' class='StatusClickHint' onclick='remove_disk(2);' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'><#Eject_usb_disk#> <span style='font-weight:normal'>USB Disk 1</span></div>";
				if((usb_path_tmp[1] != usb_path2) && usb_path2 != "usb=" && usb_path2 != "usb=printer")
					statusmenu += "<div style='margin-top:2px;' class='StatusClickHint' onclick='remove_disk(1);' onmouseout='this.className=\"StatusClickHint\"' onmouseover='this.className=\"StatusClickHint_mouseover\"'><#Eject_usb_disk#> <span style='font-weight:normal'>USB Disk 2</span></div>";
			}
...
function check_common_string(pwd, flag) {
	...
	var repeat = "";
	for(var i=0;i<pwd.length;i++) repeat += pwd.charAt(0);
	if(pwd == repeat) { nSeqString++; }
...
function chkPasswrodStrength(pwd, flag) {
	var orig_pwd = "";
	...
	if (pwd) {
		...
		if(flag == 'http_passwd'){
			...
			chkPasswrodStrength(orig_pwd, 'http_passwd');
		}
	}
}
