...
function initial(){
	show_menu();
	showfilter_lwlist();
	load_WellKnownApplications();
	check_Timefield_checkbox();
	if ('<% nvram_get("ntp_sync"); %>' == "0")
		$('svc_hint').style.display = "";
	corrected_timezone();
}

function load_WellKnownApplications(){
	wItem = new Array(new Array("WWW", "80", "TCP"), 
...
function validForm(){
	if (!validate_timerange(document.form.filter_lw_time_x_starthour, 0) 
			|| !validate_timerange(document.form.filter_lw_time2_x_starthour, 0) 
			|| !validate_timerange(document.form.filter_lw_time_x_startmin, 1) 
			|| !validate_timerange(document.form.filter_lw_time2_x_startmin, 1) 
			|| !validate_timerange(document.form.filter_lw_time_x_endhour, 2) 
			|| !validate_timerange(document.form.filter_lw_time2_x_endhour, 2) 
			|| !validate_timerange(document.form.filter_lw_time_x_endmin, 3) 
			|| !validate_timerange(document.form.filter_lw_time2_x_endmin, 3))
		return false;

	if (!validate_portlist(document.form.filter_lw_icmp_x, 'filter_lw_icmp_x'))
		return false;

	if (!document.form.filter_lw_date_x_Sun.checked 
			&& !document.form.filter_lw_date_x_Mon.checked 
			&& !document.form.filter_lw_date_x_Tue.checked 
			&& !document.form.filter_lw_date_x_Wed.checked 
			&& !document.form.filter_lw_date_x_Thu.checked 
			&& !document.form.filter_lw_date_x_Fri.checked 
			&& !document.form.filter_lw_date_x_Sat.checked 
			&& !document.form.fw_lw_enable_x[1].checked) {
...
function addRow_Group(upper){
	...
	if (document.form.filter_lw_srcport_x_0.value != "" 
			|| document.form.filter_lw_dstport_x_0.value != "")
		replace_symbol();
...
function check_duplicate(){
	var rule_num = $('filter_lwlist_table').rows.length;
	var item_num = $('filter_lwlist_table').rows[0].cells.length;
	//check same rule  //match(src ip + src port + dst ip + dst port + proto) is not accepted
	for (i=0; i<rule_num; i++) {
			...
			<table width="760px" border="0" cellpadding="4" cellspacing="0" class="FormTitle" id="FormTitle">
			  ...
			  <tr>
				<td bgcolor="#4D595D" valign="top">
				  <div>&nbsp;</div>
				  <div class="formfonttitle"><#menu8#> - <#menu8_4#></div>
				  ...
				  <div class="formfontdesc"><span id="svc_hint" onClick="location.href='Advanced_System_Content.asp?af=ntp_server0'" style="display:none;color:#FFCC00;text-decoration:underline;cursor:pointer;">* Remind: Did not synchronize your system time with NTP server yet.</span></div>
				  <div class="formfontdesc"><span id="timezone_hint" onclick="location.href='Advanced_System_Content.asp?af=time_zone_select'" style="display:none;color:#FFCC00;text-decoration:underline;cursor:pointer;"></span></div>

				  <table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3"  class="FormTable">
					<thead>
					<tr>
					  <td colspan="6" id="filter_lwlist"><#menu8_4#></td>
					</tr>
					</thead>
					...
					<tr>
					  ...
					  <td>
						<input type="radio" value="1" name="fw_lw_enable_x" onClick="return change_common_radio(this, 'FirewallConfig', 'fw_lw_enable_x', '1')" <% nvram_match("fw_lw_enable_x", "1", "checked"); %>><#checkbox_Yes#>
						<input type="radio" value="0" name="fw_lw_enable_x" onClick="return change_common_radio(this, 'FirewallConfig', 'fw_lw_enable_x', '0')" <% nvram_match("fw_lw_enable_x", "0", "checked"); %>><#checkbox_No#>
					  </td>
					</tr>
