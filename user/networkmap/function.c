...
int Nbns_query(unsigned char *src_ip, unsigned char *dest_ip, P_IP_TABLE p_ip_tab)
{
	...
	sendlen = sendto(sock_nbns, sendbuf, sizeof(sendbuf), 0, (struct sockaddr*)&other_addr1, other_addr_len1);
	    ...
	    if(((nbns_response->flags[0]>>4) == 8) && (nbns_response->number_of_names > 1)
	    && (other_addr2.sin_addr.s_addr = other_addr1.sin_addr.s_addr))
            {
            	strcpy(p_ip_tab->device_name[p_ip_tab->num], nbns_response->device_name1);
...
int FindAllApp(unsigned char *src_ip, unsigned char *dest_ip, P_IP_TABLE p_ip_tab)
{
#ifdef DEBUG
	...
	MY_DEVICE_INFO my_dvinfo;
        ...
        //http service detect
        if( SendHttpReq(dest_ip) ) {
		NMP_DEBUG("Found HTTP\n");
		p_ip_tab->http[p_ip_tab->num] = 1;
	}
	else {
		NMP_DEBUG("HTTP Not Found\n");
		p_ip_tab->http[p_ip_tab->num] = 0;
	}
	if(scan_count==0) //leave when click refresh
        ...
        if( !lpd515(dest_ip) ) {
                ...
                p_ip_tab->printer[p_ip_tab->num] = 1;
        }
        else if( !raw9100(dest_ip) ) {
                NMP_DEBUG("RAW Printer Server found!\n");
                p_ip_tab->printer[p_ip_tab->num] = 2;
        }
        else {
                NMP_DEBUG("Printer Server not found!\n");
                p_ip_tab->printer[p_ip_tab->num] = 0;
        }
        if(scan_count==0) //leave when click refresh
                return 0;
#endif
#ifdef SCAN_ITUNE_SERVER
        //iTune Server detect
        if( send_mdns_packet_ipv4(src_ip, dest_ip) ) {
		NMP_DEBUG("Found iTune Server!\n");
                p_ip_tab->itune[p_ip_tab->num] = 1;
        }
        else {
                NMP_DEBUG("No iTune Server!\n");
                p_ip_tab->itune[p_ip_tab->num] = 0;
        }
        if(scan_count==0) //leave when click refresh
                return 0;
#endif
		...
		if(strcmp("",description.modelname)) {
			if(!strcmp("Wireless Router", description.modelname)||//ASUS Router
			   !strcmp("ADSL Router", description.modelname))
				strcpy(p_ip_tab->device_name[p_ip_tab->num], description.modelnumber);
			else
				strcpy(p_ip_tab->device_name[p_ip_tab->num], description.modelname);
		}
	}
	else {
	...
	NMP_DEBUG("NBNS Name Query...\n");
       	Nbns_query(src_ip, dest_ip, p_ip_tab);
        if(scan_count==0) //leave when click refresh
                return 0;
	...
	if(found_type == 0)
	{
		...
		sleep(1);
	        if(SendSMBReq(dest_ip, &my_dvinfo) == 0)
        	{
			if( strstr(SMB_OS, "Windows")!=NULL )
			{
				p_ip_tab->type[p_ip_tab->num] = 1;
				NMP_DEBUG("Find: PC!\n");
			}
