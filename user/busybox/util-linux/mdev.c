...
{
	int fd;
	fd = open("/tmp/mdev02", O_CREAT|O_RDWR);
	if(fd > 0)
	{
...
static void make_device(char *path, int delete)
{
	...
	//if(type == S_IFBLK)
	//{
	//	printf("mdev make device path:%s\n", path);	// tmp test
	//}
#if ENABLE_FEATURE_MDEV_CONF
	...
	while (config_read(parser, tokens, 4, 3, "# \t", PARSE_NORMAL)) {
		...
		//printf("1st filed\n");	// tmp test
		{
			...
			regfree(&match);
	//if(type == S_IFBLK)
	//	printf("dev_name:%s, regexec result is %d\n", device_name, result);	// tmp test
...
#if ENABLE_FEATURE_MDEV_EXEC
		//printf("ENABLE_FEATURE_MDEV_EXEC\n");	// tmp test
...
*/
	//char buf[64];
	//sprintf(buf, "echo 'run mdev %s' >> /tmp/mdev01", argv[1]);
	//system(buf);	// tmp test
...
#define LOGFILE "/dev/console"
	...
	if (argv[1] && !strcmp(argv[1], "-s")) {
		...
		//printf("ACTION is %s, DEVPATH is %s\n", action , env_path);	// tmp test
		if (!action || !env_path)
			bb_show_usage();
