...
/* Add by Joey */
/* To limit log in local up to two 128-lines files */
//#define MAX_LOGFILE_LINES 512
#define MAX_LOGFILE_LINES 80
static char logFilePathBackup[64];
static int  logFileCount=MAX_LOGFILE_LINES;

/* circular buffer variables/structures */
#ifdef FEATURE_IPC_SYSLOG
#if __GNU_LIBRARY__ < 5
#error Sorry.  Looks like you are using libc5.
#error libc5 shm support isnt good enough.
#error Please disable FEATURE_IPC_SYSLOG
#endif
...
#endif
    /* Add by Joey to save log to different files */
    if (logFileCount==MAX_LOGFILE_LINES)
	{
        logFileCount = 0;
...
/* Add by Joey to filter out some noisy log message */
/* Currently, they are : pptp, pppoe-relay, pppd	*/
#ifndef REMOVE
	if (strstr(msg, "pptp[") || 
		strstr(msg,"pppoe[") || 
		strstr(msg,"pppoe-relay[") ||
		strstr(msg,"chat[") ||
		(strstr(msg, "pppd[") && strstr(msg, "Hangup")))
	{
		return;
	}
#endif
...
#ifdef REMOVE_WL600
			bb_error_msg_and_die("syslogd: cannot write to remote file handle on"
...
static void init_RemoteLog (void){
  ...
  if (remotefd < 0) {
    bb_error_msg_and_die("syslogd: cannot create socket");
  }
  ...
  */
  if ( 0 != (connect(remotefd, (struct sockaddr *) &remoteaddr, len))){
    bb_error_msg_and_die("syslogd: cannot connect to remote host %s:%d", RemoteHost, RemotePort);
  }
...
#ifdef REMOVE_WL600 // Joey, Don't log this message
			...
			if (FD_ISSET (fd, &readfds)) {
				...
				if (fd == sock_fd) {
					...
					if ((conn = accept (sock_fd, (struct sockaddr *) &sunx, &addrLength)) < 0) {
						bb_perror_msg_and_die ("accept error");
					}
...
extern int syslogd_main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "m:nO:R:LC:t:")) > 0) {
		switch (opt) {
			case 'm':
