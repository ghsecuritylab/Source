...
//		return;
	...
	printf("command='%s' action=%d tty='%s'\n", a->command, a->action_type, a->terminal);
}
...
static void parse_inittab(void)
{
	...
	if (parser == NULL) {	// F
		...
		//new_init_action(RESTART, "init", "");		// orig
		new_init_action(RESTART, "/sbin/init", "");	// disable for tmp
		/* Askfirst shell on tty1-4 */
		printf("new act: console\n");	// tmp test
...
int init_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (argv[1] && !strcmp(argv[1], "-q")) {
		printf("kill 1 SIGHUP\n");	// tmp test
		return kill(1, SIGHUP);
	}

	if (!ENABLE_DEBUG_INIT) {	// T
		printf("! enable debug init\n");	// tmp test
		/* Expect to be invoked as init with PID=1 or be invoked as linuxrc */
		if (getpid() != 1 && (!ENABLE_FEATURE_INITRD || !strstr(applet_name, "linuxrc"))
		) 
		{	// F
			printf("bb show usage\n");	// tmp test
			bb_show_usage();
		}
		/* Set up sig handlers  -- be sure to
		 * clear all of these in run() */
		printf("setup sig handlers\n");	// tmp test
		...
		printf("turn off rebooting via CTL-ALT-DEL, init reboot\n");	// tmp test
	...
	if (ENABLE_SWAPONOFF) {	// F
		struct sysinfo info;

		printf("enable SWAPONOFF\n");	// tmp test
		if (!sysinfo(&info) &&
			(info.mem_unit ? : 1) * (long long)info.totalram < 1024*1024)
		{
			printf("low mem, force swapon\n");	// tmp test
...
#if ENABLE_SELINUX	// F
	printf("ENABLE SELINUX\n");	// tmp test
	if (getenv("SELINUX_INIT") == NULL) {
		printf(" SELINUX_INIT is NULL\n");
		int enforce = 0;
	...
	printf("make the command line just say init\n");	// tmp test
	...
	printf("run actions SYSINIT\n");	// tmp test
	run_actions(SYSINIT);

	/* Next run anything that wants to block */
	printf("run actions WAIT\n");	// tmp test
	run_actions(WAIT);

	/* Next run anything to be run only once */
	printf("run actions ONCE\n");	// tmp test
	run_actions(ONCE);

	/* Redefine SIGHUP to reread /etc/inittab */
#if ENABLE_FEATURE_USE_INITTAB	// T
	printf("ENABLE_FEATURE_USE_INITTAB\n");	// tmp test
	signal(SIGHUP, reload_signal);
#else
	printf("! ndef ENABLE_FEATURE_USE_INITTAB\n");	// tmp test
