...
int StateTableInit(char *descDocUrl)
{
	...
	for(try_counts=0; try_counts<10; ++try_counts)
	{
		if ((ret = UpnpDownloadXmlDoc(descDocUrl, &ixmlDescDoc)) != UPNP_E_SUCCESS)
		{
			printf("[upnpd] try parse description document fail... retrying %d\n", try_counts);	// tmp test
			sleep(1);
			continue;
		}
		else
	...
	//if (strcmp(sr_event->UDN, gateUDN) == 0)
	{
		...
		if (strcmp(sr_event->ServiceId, "urn:upnp-org:serviceId:WANCommonIFC1") == 0)
		{
		    ...
		    if (GetLinkStatus() == 1)
				...
				UpnpAddToPropertySet(&propSet, "PhysicalLinkStatus", "Down");
		...
		else if (strcmp(sr_event->ServiceId, "urn:upnp-org:serviceId:L3Forwarding1") == 0)
		{
		    trace(3, "Recieved request to subscribe to L3Forwarding1");
			UpnpAddToPropertySet(&propSet, "DefaultConnectionService", "uuid:5f75f342-9f96-4ecc-9dbd-6f981efc3f14,urn:upnp-org:serviceId:WANIPConn1");
			UpnpAcceptSubscriptionExt(deviceHandle, sr_event->UDN, sr_event->ServiceId,
						  propSet, sr_event->Sid);
			ixmlDocument_free(propSet);
		}
	...
	//if (strcmp(ca_event->DevUDN, gateUDN) == 0)
	{
		...
		else if (strcmp(ca_event->ServiceID,"urn:upnp-org:serviceId:WANCommonIFC1") == 0)
		{
			if (strcmp(ca_event->ActionName,"GetTotalBytesSent") == 0)
				...
				result = X_GetICSStatistics(ca_event);
			else 
			{
...
int GetCommonLinkProperties(struct Upnp_Action_Request *ca_event)
{
   char resultStr[RESULT_LEN];
	...
	ret = GetLinkStatus();
	if (ret == 1)
		strcpy(LinkStatus, "Up");
	else if (ret == 0)
		strcpy(LinkStatus, "Down");
	else
		return 402;
	...
	snprintf(resultStr, RESULT_LEN,
		...
		"</u:GetCommonLinkPropertiesResponse>",g_vars.upstreamBitrate,g_vars.downstreamBitrate, LinkStatus);
...
int X_GetICSStatistics(struct Upnp_Action_Request *ca_event)
{
	char dev[IFNAMSIZ], resultStr[RESULT_LEN], buffer[256];
	...
	do
	{
		if (fgets(buffer, 256, proc) == NULL)
		{
			break;
		}
		read = sscanf(buffer, "%[^:]:%lu %lu %*u %*u %*u %*u %*u %*u %lu %lu %*u %*u %*u %*u %*u %*u\n", dev, &stats[STATS_RX_BYTES], &stats[STATS_RX_PACKETS], &stats[STATS_TX_BYTES], &stats[STATS_TX_PACKETS]);

		// add a fix for ra0, some interface will show "xxxx : No statistics available."
		if (read == 1)
			read = 5;
	}
	...
	snprintf(resultStr, RESULT_LEN,
		...
		"<NewTotalPacketsSent>%d</NewTotalPacketsSent>\n"
		"<NewTotalPacketsReceived>%d</NewTotalPacketsReceived>\n"
		"<NewX_Uptime>%d</NewX_Uptime>\n"
		"</u:X_GetICSStatisticsResponse>", stats[STATS_TX_BYTES], stats[STATS_RX_BYTES], stats[STATS_TX_PACKETS], stats[STATS_RX_PACKETS], uptime);
   ...
   {
      trace(1, "Error parsing Response to X_GetICSStatistics: %s", resultStr);
      ca_event->ActionResult = NULL;
      ca_event->ErrCode = 402;
   }
...
int ExpirationTimerThreadInit(void)
{
  ...
  if( ( retVal = TimerThreadInit( &gExpirationTimerThread,
  ...
  
	{
		...
		TPJobInit( &job, ( start_routine ) PeriodicallyCheck, NULL );
		TPJobSetPriority( &job, LOW_PRIORITY );
		if( ( ret = TimerThreadSchedule( &gExpirationTimerThread,
						expiretime,
						ABS_SEC, &job, SHORT_TERM,
						&eventID) ) 
			!= UPNP_E_SUCCESS )
		{
    		trace(1,"TimerThreadSchedule failed\n"); // Roger debug
		}
	}
...
#define LINK_STATUS_CHECK_PERIOD 5
static int CurrentLinkStatus = 1;
// Roger add periodical task, Doing link status check every 5 seconds
static void PeriodicallyCheck()
{
	int NewLinkStatus;
	int ret;
	...
	{
		trace(2,"Status changed to %d\n", NewLinkStatus);
		CurrentLinkStatus = NewLinkStatus;

		// Send Notify to all Subscriber
		if (NewLinkStatus == 1)
			UpnpAddToPropertySet(&propSet, "PhysicalLinkStatus", "Up");
		else
			UpnpAddToPropertySet(&propSet, "PhysicalLinkStatus", "Down");
		// Roger test
		UpnpNotifyExt(deviceHandle, "uuid:c4131cbd-0408-463d-91b0-38dd1ee22fd6", "urn:upnp-org:serviceId:WANCommonIFC1", propSet);

	}

	
	// schedule next timer
	expiretime += LINK_STATUS_CHECK_PERIOD;
...
static int GetLinkStatus()
{
	...
	if (sk < 0) {
		printf("linux-igd: Open socket failed\n");
		return -1;
	}
