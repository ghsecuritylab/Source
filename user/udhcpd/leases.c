...
{
	u_int32_t start, addr, ret;	// vlad add
...
#define hlen 6
       ...
       do {
		/* ie, 192.168.55.0 */
		if (!(addr & 0xFF)) goto next_addr;	// vlad add

		/* ie, 192.168.55.255 */
               if ((addr & 0xFF) == 0xFF) goto next_addr;	// vlad add

               /* check is an IP is used by LAN IP */
...
#if 0
#define TIME_BEFORE_2000 0xbbf81e00

/* refresh lease after ntpc */
void refresh_lease_after_ntpc(unsigned int ntpc_offset)
{
        unsigned int i;
        unsigned long offset;

        offset=atol(nvram_safe_get("ntp_offset"));
        nvram_set("ntp_offset", "0");

        if(offset==0) return;

        for (i = 0; i < server_config.max_leases; i++)
        {
                if (leases[i].expires < TIME_BEFORE_2000)
                {
                        leases[i].expires = leases[i].expires + offset;
                }
        }
}
#endif
