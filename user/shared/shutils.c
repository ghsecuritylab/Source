...
int surfing_internet(void)
{
	FILE *fp;
	char buf[128];
	int surfing;

	// check gateway of route table
	system("route -n | grep UG > /tmp/surfing");

	fp = fopen("/tmp/surfing", "r");
	if (fp == NULL) {
		system("rm -f /tmp/surfing");
		return 0;
	}
 ...
 if (now)
  {
   char *utime;
   unsigned long last;
   utime=nvram_get("u_scan_time");
   if (utime)
    last=atol(utime);
   else
    last=0;
   if(last==0) return 0; // initial value
   if ((now-last)<=120)
    return 1;
  }
 return 0;
...
void sys_default(void)
{
	...
	// nvram
	eval("/bin/mtd_write", "erase", "Config");
