         ...
         gpointer user_data)
{
    ...
    gchar *unit_val=NULL;
    char *pt1,*pt2;
    long long newpos, orgpos, offset;
    time_t cur_time;
    FILE *fp;
    char cmd_buf[512];
    char play_pid[10];
    ...
    avtransport_seek_action_get (action,
                              &instance_id,
                              &unit,
                              &unit_val);
    g_debug ("unit=%s\n",unit);
        ...
        *pt2='\0';
        ...
        if (*pt2==':') { // correct
          *pt2='\0';
          ...
          if (*pt2=='\0') { // correct
            newpos+=atoi(pt1);
            g_debug ("Seek to new position:[%ll]\n", newpos);
            cur_time=time(NULL);
            orgpos=cur_time-play_time;
            offset=newpos-orgpos;
            play_time-=offset; // correct play_time
            if (pause_time)
             pause_time-=offset; // correct pause_time
            fp = fopen("/var/run/asplayer.pid", "r"); 
            if(fp != NULL){
              fgets(play_pid,10,fp);
              fclose(fp);
            }
            fp = fopen("/tmp/seek.txt", "w"); 
            if(fp != NULL){
              fprintf(fp,"%llu",newpos);
              fclose(fp);
            }
            sprintf(cmd_buf,"kill -SIGUSR2 %d",atoi(play_pid));
	    system(cmd_buf);	
          }
        }
      }
    } else {
    // TBD. maybe there is another time format
    }
    /////
    gupnp_service_action_return (action);    
}
