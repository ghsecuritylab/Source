...
#define XML_SOURCE_FOLDER_DEFAULT "/m3player"
#define XML_TARGET_FOLDER_DEFAULT "/tmp/m3player"
#define ROOT_FILE_DEFAULT "MediaRendererV2.xml"
#define MODEL_ICON_FILE "model.png"
...
char *getMAC(void)
{
 ...
 char *pt;
 int i;
 strcpy(MAC,"00:01:02:03:04:05");
 fp=popen("ifconfig br0","r");
 if (fp)
  {
   int rlen;
   rlen=fread(cmdbuf,1,1023,fp);
   pclose(fp);
   if (rlen > 0)
    {
     pt=strstr(cmdbuf,"HWaddr ");
     if (pt)
      {
       pt+=strlen("HWaddr ");
       memcpy(MAC,pt,17);
...
filesystem_init ()
{
    ...
    g_hash_table_insert (ht, "UUMAC", getMAC());
    ...
    sprintf(src_buf,"%s/%s",xmlSource,MODEL_ICON_FILE);
    sprintf(dst_buf,"%s/%s",xmlTarget,MODEL_ICON_FILE);
    if (symlink(src_buf,dst_buf))
	    printf("ERR is %d\n",errno);
    }
