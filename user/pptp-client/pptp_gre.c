 ...
 *
 * $Id: pptp_gre.c,v 1.1.1.1 2008/07/21 09:19:28 james26_jang Exp $
 */
		   ...
		   int pty_fd, int gre_fd)
{
	...
	//fprintf(stderr, "[pptp-c] pptp_gre copy (pty_fd = %d, gre_fd = %d)\n", pty_fd, gre_fd);	// tmp test
    /* Dispatch loop */
    for (;;) { /* until error happens on gre_fd or pty_fd */
	    ...
	    {
		//fprintf(stderr, "[pptp-c] break 1\n");	// tmp test
                break;	// disable for tmp
	    }
	    ...
	    {
		//fprintf(stderr, "[pptp-c] break 2\n");	// tmp test
                break;
	    }
        }
        if (dequeue_gre (encaps_hdlc, pty_fd) < 0)
	{
	    //fprintf(stderr, "[pptp-c] break 3\n");	// tmp test
            break;
        }
    }
    /* Close up when done. */
...
#define IP_MTU 14
		...
		//fprintf(stderr, "[pptp-c] return -1\n");	// tmp test
        	return -1;	// disable for tmp
    	}
	//else
	//{ 
	//	fprintf(stderr, "[pptp-c] read success\n");	// tmp test
	//	break;
	//}
    //}
    //if(i == 20)
