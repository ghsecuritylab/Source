...
ifeq ("$(shell git log --pretty=%h -n 1 2>/dev/null)", "")
BUILD_INFO := nongit
else
...
include .config
include vendors/ASUS/$(CONFIG_PRODUCT)/version.conf 

TRX_IMAGE = $(IMAGEDIR)/$(CONFIG_PRODUCT)_$(KERNEL_VER).$(FS_VER).trx

#changed by Steven Liu
#all: ucfront cksum subdirs romfs image
all: rt_ver uClibc++_only lib_only user_only modules romfs linux image
else
all: config_error
endif

rt_ver:
	@echo '#define RT_BUILD_INFO "$(BUILD_TIME) $(BUILD_USER)@$(BUILD_INFO)"' > user/rc/version.h
...
PREBUILD_ROMFSDIR = $(ROOTDIR)/prebuild_romfs
...
ifneq ($(wildcard ../NonGPL/drivers),)
	@$(MAKE) -C ../NonGPL/drivers
endif
...
romfs_dir:
	...
	@if [ ! -e $(PREBUILD_ROMFSDIR) ] ; then \
		mkdir $(PREBUILD_ROMFSDIR);\
	fi
endif
...
.PHONY: romfs.post
romfs.post:
	-@cp -af $(PREBUILD_ROMFSDIR)/* $(ROMFSDIR)
...
.PHONY: bin_file
bin_file:
	@cp vendors/ASUS/$(CONFIG_PRODUCT)/version.conf ../bin/$(CONFIG_PRODUCT)/
	@sed -i 's/"//g' ../bin/$(CONFIG_PRODUCT)/version.conf 
	[ ! -e ../bin/$(CONFIG_PRODUCT)/Makefile ] || $(MAKE) -C ../bin/$(CONFIG_PRODUCT) O=$(IMAGEDIR) FW_FN=$(TRX_IMAGE)
